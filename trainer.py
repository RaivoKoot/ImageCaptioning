...
def train_epoch(epoch_index, dataloader, image_encoder, image_decoder, embedding_block, criterion, optimizer, device):
    image_encoder.train()
    image_decoder.train()
...
def train_step(image_batch, word_ids_batch, image_encoder, image_decoder, embedding_block, criterion, acc_metric,
    ...
    hidden_state = image_encoder.hidden_state_0.repeat((1, BATCH, 1))
...
def generate_caption(image, image_encoder, image_decoder, embedding_block, vocab):
    """
    ...
    previous_word = '<sos>'
    hidden_state = image_encoder.hidden_state_0

    while previous_word is not END_OF_SENTENCE_WORD:
        input_word_id = vocab.word_to_index(previous_word)
        input_word_tensor = torch.tensor([input_word_id])[None, None, ...]  # add sequence and batch dimension
